...
def get_players(search: Optional[str] = None, limit: int = 50, offset: int = 0):
    if search:
        ...
        query = "SELECT id as person_id, full_name as display_first_last, first_name, last_name FROM player LIMIT ? OFFSET ?"
        params = [limit, offset]
...
def get_player(player_id: str):
    ...
    try:
        ...
        query_fallback = "SELECT id as person_id, full_name as display_first_last, first_name, last_name FROM player WHERE id = ?"
...
def get_player_stats(player_id: str):
    ...
    """
    try:
        # Note: The column names in the table might be slightly different (e.g. 'seas_id' vs 'season_id').
        # Based on previous inspection, it was 'seas_id'. Let's check inspect_game_stats.py output again.
        # It showed 'seas_id'.
        ...
        query = "SELECT * FROM player_stats_per_game WHERE person_id = ? ORDER BY seas_id DESC"
