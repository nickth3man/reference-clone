...
2.  **Frontend Configuration & Linting**: Fix ESLint/Prettier configuration to allow `trunk check`
    to run successfully. Execute `trunk fmt` and fix remaining linting issues.
3.  **Backend Linting**: Fix Python linting errors (imports, line lengths, `print` statements,
    exception handling) using `trunk check` as a guide.
4.  **Infrastructure Hardening**: Update Dockerfiles with security best practices (HEALTHCHECK,
    non-root user).
5.  **Verification**: Run full `trunk check --all` and verify clean output. Validate server startup.
